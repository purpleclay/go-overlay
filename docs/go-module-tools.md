...
| Build caching            | project-local; decoupled from your application source, so the host tool rebuilds only when a relevant derivation input changes — `go.mod`/`go.sum`, the selected `go` toolchain, or `vendorEnv` (local replacement sources, workspace metadata)                                                                                                                       | content-addressed on the tool itself, shareable across projects via a binary cache                                                          |
