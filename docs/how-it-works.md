...
1. **`go mod vendor -o <tmp>`** — one invocation. This is the platform-independence trick: `go mod vendor` performs package attribution with _every build constraint treated as satisfied_ (`imports.AnyTags` in `cmd/go`). One pass covers every `GOOS`/`GOARCH` pair Go recognises and every custom build tag, simultaneously — a strict superset of any finite platform list. Packages guarded by `//go:build windows`, `//go:build js && wasm`, or `//go:build yourcustomtag` are all attributed, unconditionally. The same pass covers `tool` directives, since Go includes tool dependencies in the pattern it vendors. Any module not already in the local module cache is downloaded as part of this same pass — there is no separate download step.
2. **Hash as you go.** `go mod vendor -v` streams a module's `modules.txt` header the moment it's committed to that module, well before the command itself exits. govendor computes that module's on-disk cache directory directly — matching the Go toolchain's own layout and escaping convention — and starts hashing it immediately, overlapping NAR hashing with the rest of vendoring rather than waiting for it to finish. Hashes are reused from the previous manifest for any module whose version is unchanged: a module version's content is immutable, so re-hashing it is wasted work. Only new or upgraded modules are hashed. Local `replace` targets are the exception — their content is mutable, so they are hashed on every run.
...
Workspace projects follow the same shape from the workspace root: a single `go work vendor` pass provides both workspace-level version selection (full MVS across all members) and attribution for all members simultaneously, with member modules recorded as local, non-fetched entries whose hashes are retained for drift detection. Tool directives from member `go.mod` files are aggregated into one `[tool]` table.
...
- **Cold generation** is dominated by network fetches, folded into the single `go mod vendor` pass, and NAR hashing, which overlaps with that same pass rather than following it. Package attribution happens in that one pass regardless of how many platforms you will ever target.
- **Warm regeneration** — the everyday case — reuses hashes for unchanged module versions. Bumping one dependency costs one vendor pass and one hash.
