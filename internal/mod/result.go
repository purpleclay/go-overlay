...
func resultGeneratorMismatch(path, manifestVersion, currentVersion string) vendorResult {
	msg := fmt.Sprintf("govendor.toml was generated by %s, current version is %s\n  run 'govendor' to regenerate, or use --check --strict to fail on version warnings", manifestVersion, currentVersion)
	return vendorResult{path: path, status: statusWarning, message: msg}
}

func resultGeneratorMajorMismatch(path, manifestVersion, currentVersion string) vendorResult {
	msg := fmt.Sprintf("govendor.toml was generated by %s, current version %s is incompatible — run 'govendor' to regenerate", manifestVersion, currentVersion)
	return vendorResult{path: path, status: statusDrift, message: msg}
}
