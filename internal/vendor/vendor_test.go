...
func TestVendor(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			deps, err := resolver.ResolveModule(context.Background(), goMod, nil)
			require.NoError(t, err)
			...
			manifest := vendor.New(deps, nil, tool, goMod.Excludes)
...
func TestVendorWorkspace(t *testing.T) {
	tests := []struct {
		...
		{name: "workspace", dir: "testdata/workspace"},
		{name: "workspace-with-excludes", dir: "testdata/workspace-with-excludes"},
		{name: "workspace-mvs-conflict", dir: "testdata/workspace-mvs-conflict"},
		{name: "workspace-remote-replace", dir: "testdata/workspace-remote-replace"},
		{name: "workspace-with-tools", dir: "testdata/workspace-with-tools"},
		{name: "workspace-local-replace", dir: "testdata/workspace-local-replace"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			deps, err := resolver.ResolveWorkspace(context.Background(), goWork, nil)
			require.NoError(t, err)
			...
			manifest := vendor.New(deps, goWork.WorkspaceConfig(), tool, excludes)
...
func (f *fakeResolver) ResolveModule(_ context.Context, _ *mod.GoModFile, _ map[string]mod.ModuleConfig) ([]mod.ModuleConfig, error) {
	return f.deps, nil
}

func (f *fakeResolver) ResolveWorkspace(_ context.Context, _ *mod.GoWorkFile, _ map[string]mod.ModuleConfig) ([]mod.ModuleConfig, error) {
	return f.deps, nil
}
...
func TestVendorWithCheck_DriftSchemaMismatch(t *testing.T) {
	// A schema 3 manifest triggers a mismatch result under --check.
...
func TestVendor_Schema3ManifestUpgradedToSchema4(t *testing.T) {
	// A schema 3 manifest triggers regeneration (not an error) in generate mode.
	schema3 := "# Generated by govendor. DO NOT EDIT.\n\nschema = 3\n\n[mod]\n" +
	...
	dir := setupModDir(t, map[string]string{"govendor.toml": schema3})
	...
	data, err := os.ReadFile(filepath.Join(dir, "govendor.toml"))
	require.NoError(t, err)
	assert.Contains(t, string(data), "schema = 4")
}
